...
def translation_performance(txt_ref, txt_hyp, tokenizer_args: str = '13a'):
    from rouge import Rouge as SLT_Rouge
    rouge=SLT_Rouge()
    # `rouge` will raise ValueError when any hypothesis is empty string.
    # This can happen early in training when the model outputs only special tokens
    # (e.g. <extra_id_0>) which become empty after `skip_special_tokens=True`.
    try:
        scores = rouge.get_scores(txt_hyp, txt_ref, avg=True)
        scores['rouge-l']['f'] = scores['rouge-l']['f']*100
        rouge_l_f = float(scores['rouge-l']['f'])
    except ValueError:
        rouge_l_f = 0.0
