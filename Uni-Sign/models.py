...
def _allow_trusted_local_torch_load():
    """Bypass the Transformers torch.load version gate for trusted local weights.

    This repository loads a vendored local mT5 checkpoint from `pretrained_weight/mt5-base`.
    In the current runtime, `transformers` blocks `torch.load` for torch<2.6 before it even
    attempts to read the file, which prevents the web demo from starting on the devbox.
    The checkpoint is part of the repository assets, so we can safely skip this guard here.
    """
...
class Uni_Sign(nn.Module):
    def __init__(self, args):
        ...
        _allow_trusted_local_torch_load()
    ...
    def maybe_autocast(self):
        """Autocast helper for pose-rgb fusion.

        The original implementation always enabled autocast, which can introduce
        NaNs for the RGB fusion branch (especially when the user sets --dtype fp32).
        We only enable autocast when:
        - CUDA is available, and
        - args.dtype is fp16/bf16 (not fp32)
        """
        if (not torch.cuda.is_available()):
            return contextlib.nullcontext()

        dtype_flag = str(getattr(self.args, 'dtype', 'bf16')).lower()
        if dtype_flag in {'fp32', 'float32'}:
            return contextlib.nullcontext()
    ...
    def forward(self, src_input, tgt_input):
        ...
        tgt_input_tokenizer = self.mt5_tokenizer(
            ...
            padding=True,
            truncation=True,
            max_length=getattr(self.args, "tgt_max_length", 50),
        )
    ...
    @torch.no_grad()
    def generate(self, pre_compute_item, max_new_tokens, num_beams, **gen_kwargs):
